...
#+BEGIN_SRC emacs-lisp
  ...
  (defun kess-open-line-below-or-append (&optional ch)
    "Insert a newline and move point to it use
  `indent-new-comment-line' when CH is nil, otherwise append CH to
  the end of line."
    (interactive)
    (move-end-of-line 1)
    (let ((tr (buffer-substring-no-properties (point) (1- (point)))))
      (if (and (stringp ch) (eq 1 (length ch)))
          (unless (string-equal ch tr)
            (insert ch))
