...
#+BEGIN_SRC emacs-lisp
  (defun kess--switch-buffer (&optional prev)
    "Switch to non *[buffer]*. (idea from stackoverflow)

  Only switch to non *[buffer]* buffer, use next-buffer when NEXT
  is t, otherwise, previous-buffer.

  NOTE: Some special *[buffer]* are included to switching, like
  ,*scratch*, *info*, etc."
    (let ((bread-crumb (buffer-name))
          (switch-fn (or (and prev 'previous-buffer) 'next-buffer))
          (white-list '("*scratch*" "*info*" "*eshell*")))
      (funcall switch-fn)
      (while (and (not (equal bread-crumb (buffer-name)))
                  (and (not (member (buffer-name) white-list))
